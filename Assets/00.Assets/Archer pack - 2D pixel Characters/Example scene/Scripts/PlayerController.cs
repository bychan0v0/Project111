...
namespace SmallScaleInteractive._2DCharacter
{
    public class PlayerController : MonoBehaviour
    {
        ...
        public bool isArcher; //if the character is a ranged class
        public bool isActive; //if the character is active (adjusted with the ui toggles)
        ...
        // This method will be called by the toggle UI, it sets the character to active.
        ...
        void Update()
        {
            if (EventSystem.current.IsPointerOverGameObject()) //if a ui element was clicked, we exit the method.
            {
                return;
            }
...
private void HandleAttacking()
{
    if (Input.GetMouseButtonDown(0) && isGrounded) // Left mouse button for attack
    {
        ...
        int animatorDirection = animator.GetInteger("Direction"); // Get direction from animator, 0 for right, 1 for left
        float attackDirection = (animatorDirection == 1) ? -1f : 1f; // Convert animator direction to vector direction

        if (isArcher && arrowPrefab != null)
        {
            StartCoroutine(DelayedFireArrow(attackDirection, 0.4f, wasMovingWhenAttacked)); // Use attackDirection determined by animator
            animator.SetTrigger(wasMovingWhenAttacked ? "isRunAttack" : "isAttack1");
        }
        else
        {
...
IEnumerator DelayedFireArrow(float direction, float delay, bool wasMovingWhenAttacked)
{
    yield return new WaitForSeconds(delay);

    // Calculate position offset based on whether the player was moving
    Vector3 positionOffset = wasMovingWhenAttacked ? Vector3.zero : new Vector3(0, 0.16f, 0);

    // Instantiate arrow at adjusted position
    if(isActive)
    {
        GameObject arrow = Instantiate(arrowPrefab, transform.position + positionOffset, Quaternion.identity);

        // Determine the initial velocity based on the direction
        float arrowSpeed = 10f; // Adjust speed as necessary
        Vector2 launchVelocity = new Vector2(arrowSpeed * direction, 0);
        ...
        // Optionally, rotate the arrow based on the direction
        if (direction < 0)
        {
            arrow.transform.localScale = new Vector3(-1, 1, 1); // Flip arrow if shooting left
        }
    }
}
        ...
        void HandleGroundCollision()
        {
            ...
            if (isAirSlamming && isGrounded)
            {
                ...
                isAirSlamming = false;
                 // Instantiate the AoE prefab and start the destruction coroutine, if the character is active
                if (isActive)
                {
        ...
        IEnumerator ClimbOntoLedge(Bounds ledgeBounds, int direction)
        {
            ...
            yield return new WaitForSeconds(0.45f); // Adjust time based on your animation
            ...
            yield return new WaitForSeconds(0.5f);  // Additional wait time
